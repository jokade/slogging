...
object FilterLogger {
  ...
  /**
   * The filter function to bes used by all [[FilterLogger]] instances.
...
final class FilterLogger(name: String) extends AbstractUnderlyingLogger {
  ...
  override def error(source: String, message: String, args: AnyRef*): Unit = FilterLogger.filter((LogLevel.ERROR,name)).error(source,message,args:_*)
  ...
  override def warn(source: String, message: String, args: AnyRef*): Unit = FilterLogger.filter((LogLevel.WARN,name)).warn(source,message,args:_*)
  ...
  override def info(source: String, message: String, args: AnyRef*): Unit = FilterLogger.filter((LogLevel.INFO,name)).info(source,message,args:_*)
  ...
  override def debug(source: String, message: String, args: AnyRef*): Unit = FilterLogger.filter((LogLevel.DEBUG,name)).debug(source,message,args:_*)
  ...
  override def trace(source: String, message: String, args: AnyRef*): Unit = FilterLogger.filter((LogLevel.TRACE,name)).trace(source,message,args:_*)
